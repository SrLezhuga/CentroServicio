...
require "modelo_excel.php";
    ...
    $cantidad=htmlspecialchars($_POST['cant'],ENT_QUOTES,"UTF-8");
    ...
    $array_unidad=explode(",",$unidad);
    $array_costo=explode(",",$costEx);

    for ($i=0; $i < count($array_codigo); $i++) { 
        if ($array_id[$i]=="NUEVO") {
            $consulta = $ME->RegistrarNvo_Excel($array_codigo[$i],$array_descripcion[$i],$array_marca[$i],$array_cantidad[$i],$array_unidad[$i],$array_costo[$i]);
        }else {
            $consulta = $ME->Registrar_Excel($array_id[$i],$array_codigo[$i],$array_descripcion[$i],$array_marca[$i],$array_cantidad[$i],$array_unidad[$i],$array_costo[$i]);
        }
    }
    echo $consulta;
?>