...
require_once('../../conexion/conec.php');
  ...
  $db=new Conexion();
    ...
    $Fecha=date("Y-m-d");
    $rs = $db->query("SELECT MAX(folio) AS id FROM tab_orden_reparo");
   if ($row = $rs->fetch_row()) {
   $id = trim($row[0]);
...
class MYPDF extends TCPDF {
  ...
  public function Header() {
    ...
    // Title
    $this->Cell(135, 20,'MAYOREO FERRETERO ATLAS SA DE CV', 0, false, 'C', 0, '', 0, false, 'M', 'M');
    $this->Ln(5);
...
<table border="1">
  ...
  <tr>
    ...
    <td>'.$tres['status_orden'].' '.$tres['fecha_entrada'].'</td>
