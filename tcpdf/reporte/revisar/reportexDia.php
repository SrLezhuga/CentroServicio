...
$ticketactivos="SELECT folio, importe FROM `tab_ticket` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
$res=mysql_query($ticketactivos);

// saca la suma de los ticket activos
$Sumaactivo="SELECT round(sum(importe),2) as total FROM `tab_ticket` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
$ticketsuma=mysql_query($Sumaactivo);
$sumtikect=mysql_fetch_array($ticketsuma);

//ticlet cancelados
$cancelados="SELECT folio, importe FROM `tab_ticket` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='C'";
$cancelados1=mysql_query($cancelados);

//sumad de ticket cancelados
$Sumacance="SELECT round(sum(importe),2) as total FROM `tab_ticket` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='C'";
...
$facturaactivos="SELECT a.folio, a.importe, tab_refacturacion.descripcion FROM tab_factura as a LEFT OUTER JOIN tab_refacturacion ON a.folio = tab_refacturacion.folio_new where date(fecha)='$fecha' and a.tab_empresa_clave_empresa='$Empresa' and a.estado='A' ";
$factura=mysql_query($facturaactivos);

//saca la suma de las facturas activos
$factusum="SELECT round(sum(importe),2) as total FROM `tab_factura` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
$facturasuma=mysql_query($factusum);
$sumfactura=mysql_fetch_array($facturasuma);

//factura cancelados
$facturacancelados="SELECT folio, importe FROM `tab_factura` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' and estado!='A'  ";
$facturacancelados1=mysql_query($facturacancelados);
 
 // sumas de las facturas canceladas

$Sumacancefactura="SELECT round(sum(importe),2) as total FROM `tab_factura` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'   and estado!='A' ";
...
$tarjetaactivos="SELECT No_movimiento, importe, descripcion FROM tab_tarjeta where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' and estado='A' ";
$tarjeta=mysql_query($tarjetaactivos);

//saca la suma de las tarjetas activos
$tarjetasum="SELECT round(sum(importe),2) as total FROM `tab_tarjeta` where date(fecha)='$fecha'  and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
$tarjetasuma=mysql_query($tarjetasum);
$sumtarjeta=mysql_fetch_array($tarjetasuma);

//tarjetas cancelados
$tarjetacancelados="SELECT No_movimiento, importe, descripcion FROM `tab_tarjeta` where date(fecha)='$fecha'  and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
$tarjetacancelados1=mysql_query($tarjetacancelados);
 
 // sumas de las tarjetas canceladas

$Sumacancetarjeta="SELECT round(sum(importe),2) as total FROM `tab_tarjeta` where date(fecha)='$fecha'  and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
...
$chequeactivos="SELECT No_movimiento, importe, descripcion FROM tab_cheques  where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' and estado='A' ";
$cheque=mysql_query($chequeactivos);

//saca la suma de las cheques activos
$chequesum="SELECT round(sum(importe),2) as total FROM `tab_cheques` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
$chequesuma=mysql_query($chequesum);
$sumcheque=mysql_fetch_array($chequesuma);

//cheque cancelados
$chequecancelados="SELECT No_movimiento, importe, descripcion FROM `tab_cheques` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
$chequecancelados1=mysql_query($chequecancelados);
 
 // sumas de las cheques canceladas

$Sumacancecheque="SELECT round(sum(importe),2) as total FROM `tab_cheques` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
...
$transferenciaactivos="SELECT No_movimiento, importe, descripcion FROM tab_transferencia  where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' and estado='A'";
$transferencia=mysql_query($transferenciaactivos);

//saca la suma de las transferencias activos
$transferenciasum="SELECT round(sum(importe),2) as total FROM `tab_transferencia` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='A'";
...
$transferenciacancelados="SELECT No_movimiento, importe, descripcion FROM `tab_transferencia` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
$transferenciacancelados1=mysql_query($transferenciacancelados);
 
 // sumas de las transferencia canceladas

$Sumacancetrans="SELECT round(sum(importe),2) as total FROM `tab_transferencia` where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'  and estado='R'";
...
$movimiento="SELECT descripcion,importe FROM tab_movimientos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$movimietos1=mysql_query($movimiento);

$movimientosum="SELECT round(sum(importe),2) as total FROM tab_movimientos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
...
$salida="SELECT nombre,importe FROM tab_retiro where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$salida1=mysql_query($salida);

$salidasum="SELECT round(sum(importe),2) as total FROM tab_retiro where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$salidasuma=mysql_query($salidasum);
$salidasuma1=mysql_fetch_array($salidasuma);

//deposito

$deposito="SELECT folio,importe,descripcion FROM tab_deposito where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$deposito1=mysql_query($deposito);

$depositosum="SELECT round(sum(importe),2) as total FROM tab_deposito where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$depositosuma=mysql_query($depositosum);
$depositosuma1=mysql_fetch_array($depositosuma);

//credito

$credito="SELECT No_movimiento,importe,descripcion FROM tab_credito where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
$credito1=mysql_query($credito);

$creditosum="SELECT round(sum(importe),2) as total FROM tab_credito where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
...
//
 ...
 // saldo inicial
$saldoini="select round(sum(saldo_inicial),2) as ti from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'";
$saldoini1=mysql_query($saldoini);
$saldoini2=mysql_fetch_array($saldoini1); 

 // total de ventas

$totalventas="select round(sum(total_ventas),2) as tv from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'";
$totalventas1=mysql_query($totalventas);
$totalventas2=mysql_fetch_array($totalventas1); 

 // total de gatos

$totalgasto="select round(sum(total_gastos),2) as tg from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'";
$totalgasto1=mysql_query($totalgasto);
$totalgasto2=mysql_fetch_array($totalgasto1); 

// total de ventas

$totalfinal="select round(sum(saldo_final),2) as sf from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'";
$totalfinal1=mysql_query($totalfinal);
$totalfinal2=mysql_fetch_array($totalfinal1); 

// venbta total

$ventatotal="select round(sum(venta_total),2) as vt from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa'";
...
//$saldo="select *from tab_no_corte where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
...
// {
 ...
 $salini="select saldo_inicial as saldoi from tab_saldos where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' and no_corte='1'";
...
//else
//{
 //$sinicial="select *from tab_saldoini where date(fecha)='$fecha' and tab_empresa_clave_empresa='$Empresa' ";
